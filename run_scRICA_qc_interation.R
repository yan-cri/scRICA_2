## R script used to conduct scRNA-Seq comparative integration analysis                  ##
## step1: identify doublets with findDoublets() Function in library(scRICA)             ##
## step2: pre-process cellranger count results for QC with countReadin() Function in library(scRICA)             ##
## Developed by Yan Li, 1st version on March, 2021, updated on July, 2021 with scRICA including expCond1/2  ##
##--------------------------------------------------------------------------------------##
...
## countReadin() changed into processQC()
...
# load(sprintf('%s/seuratReadin_qcProcessed.Rdata', args$outputDirName))
# print("load qc processed Rdata")
# print(seuratProcssedObjList)
# seuratProcssedObjList$resDir <- outputDir
# print(sprintf("updated resDir is %s", seuratProcssedObjList$resDir))
## ---------
# ?getClusterMarkers() changed into getIntegrationClusterMarkers()
seuratIntegratedRes     <- getClusterMarkers(qcProcessedResults = seuratProcssedObjList, integrationMethod = as.character(args$integration.method), topN = 50)
